// lib/summaryStats.js
const { getRepoDependencyUpdates } = require('./dependencyUpdates')
const { getSeverity } = require('./severity')
const { getPackageJson } = require('./github')
const { getLatestLtsVersion } = require('./ltsVersion')

async function getNodeVersionStats (repoNames) {
  ...
  for (const repo of repoNames) {
    try {
      ...
      const nodeVersion = typeof pkg.engines?.node === 'string' ? pkg.engines.node : 'unknown'
      const severity = nodeVersion && nodeVersion !== 'unknown'
...
module.exports = { getSummaryStats, getNodeVersionStats }
